#!/usr/bin/env bash
#
# Puts an Apple signing identity and its provisioning profiles onto a CI runner.
#
#   ./scripts/ci-apple-signing.sh setup macos
#   ./scripts/ci-apple-signing.sh teardown macos
#   ./scripts/ci-apple-signing.sh setup ios
#   ./scripts/ci-apple-signing.sh teardown ios
#
...
#
#   iOS
#     IOS_CERTIFICATE_P12          Apple Distribution, exported from Keychain Access
#     IOS_CERTIFICATE_PASSWORD     the password set during that export
#     IOS_PROVISIONING_PROFILES    base64 of a zip of the .mobileprovision files
#
# base64 because a GitHub secret is a string and a .p12 is not text. Read them
# back with: base64 --decode <<< "$SECRET" > file.p12
#
# The two platforms differ in three details and nothing else -- the file
# extension a profile carries, the name of the identity that has to survive the
# import, and which secrets it reads. They are named here rather than kept as a
# second copy of the script, because the parts that are easy to get wrong (the
# trap, the recorded profile list, the partition list) are the parts that would
...
case "$PLATFORM" in
  macos)
    ...
    REQUIRED_IDENTITY="Developer ID Application"
    CERTIFICATE="${MACOS_CERTIFICATE_P12:-}"
    CERTIFICATE_PASSWORD="${MACOS_CERTIFICATE_PASSWORD:-}"
    PROFILES="${MACOS_PROVISIONING_PROFILES:-}"
    CERTIFICATE_SECRET="MACOS_CERTIFICATE_P12"
    ;;
  ios)
    PROFILE_EXTENSION="mobileprovision"
    # Not "iPhone Distribution": that is the old name, and a certificate issued
    # today is an Apple Distribution one. A Developer ID certificate imports
    # perfectly well here and cannot upload anything, which is the mistake this
    # name exists to catch.
    REQUIRED_IDENTITY="Apple Distribution"
    CERTIFICATE="${IOS_CERTIFICATE_P12:-}"
    CERTIFICATE_PASSWORD="${IOS_CERTIFICATE_PASSWORD:-}"
    PROFILES="${IOS_PROVISIONING_PROFILES:-}"
    CERTIFICATE_SECRET="IOS_CERTIFICATE_P12"
    ;;
  *)
    echo "usage: $0 {setup|teardown} {macos|ios}" >&2
    exit 2
    ;;
esac

# Per-platform names, so that a runner which set up both does not have one
# teardown remove the other's keychain and leave its own behind.
KEYCHAIN="${RUNNER_TEMP:-/tmp}/caelo-signing-$PLATFORM.keychain-db"
...
INSTALLED="${RUNNER_TEMP:-/tmp}/caelo-installed-profiles-$PLATFORM"
...
setup() {
  [[ -n "$CERTIFICATE" ]] || {
    echo "error: $CERTIFICATE_SECRET is not set; see docs/signing.md" >&2
    exit 1
  }
  ...
  import_identity "$work" application "$CERTIFICATE" "$CERTIFICATE_PASSWORD"
  ...
  if ! grep -q "\"$REQUIRED_IDENTITY" <<< "$identities"; then
    echo "error: no $REQUIRED_IDENTITY identity in the imported certificate." >&2
...
install_profiles() {
  ...
  if [[ -z "$PROFILES" ]]; then
    echo "!! no provisioning profiles; the extension will not build" >&2
    return
  fi

  base64 --decode <<< "$PROFILES" > "$work/profiles.zip"
  ...
  while IFS= read -r -d '' profile; do
    ...
    for directory in "${PROFILE_DIRS[@]}"; do
      cp "$profile" "$directory/$uuid.$PROFILE_EXTENSION"
      # Recorded, not matched by pattern later: teardown must remove what this
      # run put there and leave anything else alone.
      printf '%s\n' "$directory/$uuid.$PROFILE_EXTENSION" >> "$INSTALLED"
    done
    # The name, not the file name: PROVISIONING_PROFILE_SPECIFIER in the project
    # and the provisioningProfiles map in ExportOptions both match on it, so a
    # mismatch here is the whole failure.
    echo "==> Installed profile: $(security cms -D -i "$profile" 2>/dev/null |
      plutil -extract Name raw -o - - 2>/dev/null || echo "$uuid")"
    installed=$((installed + 1))
  done < <(find "$work/profiles" -name "*.$PROFILE_EXTENSION" -print0)

  [[ $installed -gt 0 ]] || echo "!! the archive contained no .$PROFILE_EXTENSION files" >&2
}
...
case "$ACTION" in
  setup) setup ;;
  teardown) teardown ;;
  *) echo "usage: $0 {setup|teardown} {macos|ios}" >&2; exit 2 ;;
esac
